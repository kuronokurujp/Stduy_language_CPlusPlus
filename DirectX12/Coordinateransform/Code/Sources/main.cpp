...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
    ...
    ID3D12RootSignature* p_root_sig = nullptr;
    {
        ...
        // ディスクリプタレンジを作成
        D3D12_DESCRIPTOR_RANGE desc_tbl_ranges[2] = {};
    ...
    // 基本ティスクリプタヒープを作る
    auto p_basic_desc_heap = Common::CreateDescriptorHeap(p_dev, 2);
    ...
    {
        // 行優先なので注意
        // 正規化デバイス座標系からスクリーン座標系に座標変換するための行列
    ...
    */
    ...
    {
        // ワールド行列
        world_mat = DirectX::XMMatrixRotationY(DirectX::XM_PIDIV4);

        DirectX::XMFLOAT3 eye(0.0, 0.0f, -5.0);
        DirectX::XMFLOAT3 target(0.0, 0.0f, 0.0);
    ...
    DirectX::XMMATRIX* p_coordinate_transform_mat = nullptr;
    {
        // 定数バッファを作成
        p_mat_constant_buff = Constant::CreateBuffer<DirectX::XMMATRIX>(p_dev, mat);
        // バッファにデータを書き込む
        {
            // バッファと書き込む先との関連付け
    ...
    // ディスクリプタヒープに各リソースビューを作成
    {
        ...
        // テクスチャーのシェーダーリソースビューを作る
        {
            ...
            auto result = Texture::CreateShaderResourceView(
                p_dev, p_tex_buff, texture_metadata.format, basic_heap_handle);
            assert(result);

            // 設定したリソースビューのサイズ分ずらして次に設定するビューの位置を決める
            basic_heap_handle.ptr += p_dev->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
        }
    ...
    while (true)
    {
        ...
        {
            // Y軸回転アニメ
            world_mat *= DirectX::XMMatrixRotationY(DirectX::XM_PIDIV4 * 0.1f);
            *p_coordinate_transform_mat = mat * world_mat * view_mat * proj_mat;
        }
        // ---------------------------------------------------------------
