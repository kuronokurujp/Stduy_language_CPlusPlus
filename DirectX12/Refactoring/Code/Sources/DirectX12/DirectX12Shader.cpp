...
namespace DirectX12
{
    namespace Impl
    {
        static const HRESULT SyncLoadShader(
            ...
            const std::string& in_r_targeet_name)
        {
    ...
    /// <returns></returns>
    const HRESULT SyncLoadShaderVS(std::shared_ptr<Context> in_p_context, const std::string& in_r_key_name, ID3D10Blob** out_pp_error_blob, const WCHAR* in_shader_filepath)
    {
    ...
    /// <param name="in_shader_filepath"></param>
    /// <returns></returns>
    const HRESULT SyncLoadShaderPS(std::shared_ptr<Context> in_p_context, const std::string& in_r_key_name, ID3D10Blob** out_pp_error_blob, const WCHAR* in_shader_filepath)
    {
        assert(in_p_context != nullptr);
        assert(out_pp_error_blob != nullptr);
        assert(!in_r_key_name.empty());
