...
namespace DirectX12
{
    ...
    /// テクスチャーバッファを作成
    /// </summary>
    Context::ComPtr<ID3D12Resource> CreateTextureBuffer(
        ...
        D3D12_RESOURCE_DIMENSION in_dimension)
    {
        ...
        Context::ComPtr<ID3D12Resource> p_res;
    ...
    /// <returns></returns>
    Context::ComPtr<ID3D12Resource> CreateUploadResoucre(
        std::shared_ptr<Context> in_p_context,
        UINT64 in_width)
    {
    ...
    /// <returns></returns>
    Context::ComPtr<ID3D12Resource> CreateCopyTextureResoucre(
        std::shared_ptr<Context> in_p_context,
    ...
    /// <param name="in_p_dev"></param>
    /// <returns></returns>
    Context::ComPtr<ID3D12Resource> CreateTextureResourceFromColor(
    ...
    /// <returns></returns>
    Context::ComPtr<ID3D12Resource> CreateTextureResourceFromGradation(
        std::shared_ptr<Context> in_p_context, UINT8 in_color)
    {
        auto p_buff = CreateTextureBuffer(
            in_p_context,
    ...
    /// <returns></returns>
    Context::ComPtr<ID3D12Resource> CreateTextureResourceFromLoadTextureFile(
        std::shared_ptr<Context> in_p_context, const std::string& in_r_file_path)
    {
        auto it = in_p_context->_tex_res_map.find(in_r_file_path);
        ...
        auto result = SyncLoadTextureFile(&metadata, &scratch_img, in_r_file_path);
        assert(result);
        ...
        in_p_context->_tex_res_map[in_r_file_path] = p_buff;
