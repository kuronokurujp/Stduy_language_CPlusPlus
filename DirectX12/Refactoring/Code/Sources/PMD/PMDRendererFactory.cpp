...
namespace PMD
{
	namespace Render
	{
			...
			const DirectX::XMFLOAT3& in_r_cam_pos)
		{
			...
			// シェーダーに受け渡すパラメータ
			{
				this->_p_scene_shader_param->world_mat = in_r_local_mat * in_r_world_mat;
			...
			const std::string& in_r_toon_path_fmt)
		{
							...
							if (0 < std::strlen(pmd_material[i].tex_file_path))
							{
								...
								for (auto& load_file_path : load_file_paths)
								{
									...
									auto load_tex_file_path_str =
										Common::CombineDirPath(in_r_pmd_filepath, load_file_path.c_str());
			...
			// しかしマザボのOnGPUだとディスクリプタヒープは1つしか作れないので注意
			{
				// 基本ディスクリプタヒープ作成
				{
					...
					auto b_result = DirectX12::CreateDescripterHeap(in_context, _renderer->_scene_desc_heap_share_key, desc_heap_desc);
					assert(b_result);
				}
			...
			// シェーダーロード
			{
				...
				HRESULT h_result;
				{
					...
					h_result = DirectX12::LoadShaderVSBySync(in_context, this->_vs_shader_key, &out_p_error_blob, filepath.c_str());
					if (FAILED(h_result))
					{
						// エラー表示
						std::string error;
...

				{
					...
					h_result = DirectX12::LoadShaderPSBySync(in_context, this->_ps_shader_key, &out_p_error_blob, ps_file_path.c_str());
					if (FAILED(h_result))
					{
						// エラー表示
						std::string error;
			...
			{
				auto basic_heap = in_context->desc_heap_map[_renderer->_scene_desc_heap_share_key.c_str()];
