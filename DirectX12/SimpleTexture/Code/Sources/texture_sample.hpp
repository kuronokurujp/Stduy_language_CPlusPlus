...
namespace Texture
{
	...
	/// <returns></returns>
	static ID3D12DescriptorHeap* CreateDescriptorHeap(ID3D12Device* in_p_dev)
	{
	...
	static bool CreateShaderResourceView(
		ID3D12Device* in_p_dev,
		ID3D12Resource* in_p_tex_buff, ID3D12DescriptorHeap* in_p_tex_desc_heap,
		DXGI_FORMAT in_rgba_format)
	{
		assert(in_p_dev != nullptr);
		assert(in_p_tex_buff != nullptr);
		assert(in_p_tex_desc_heap != nullptr);
		...
		in_p_dev->CreateShaderResourceView(
			...
			&srv_desc,
			// リソースビューを割り当てるヒープを指定
			in_p_tex_desc_heap->GetCPUDescriptorHandleForHeapStart());
