...
namespace DirectX12
{
    ...
    /// <returns></returns>
    const HRESULT CreateEnvironment(std::shared_ptr<Context> in_context)
    {
    ...
    /// </summary>
    /// <param name="in_context"></param>
    void Delete(std::shared_ptr<Context> in_context)
    {
        assert(in_context != nullptr);
    ...
    const size_t GetAdapterdx(std::shared_ptr<Context> in_context, const std::wstring in_adapter_name)
    {
    ...
    /// <returns></returns>
    bool CreateDescripterHeap(std::shared_ptr<Context> in_p_context, const std::string& in_r_key, const D3D12_DESCRIPTOR_HEAP_DESC& in_desc)
    {
    ...
    /// Blobからエラーメッセージを出力
    /// </summary>
    const bool OutputErrorMessageForBlob(std::string* out_p_error, const HRESULT in_result, ID3DBlob* const in_p_error_blob)
    {
        if (!FAILED(in_result))
            return false;
    ...
    /// <returns></returns>
    DirectX::XMMATRIX LockAtMatrixByZAxis(const DirectX::XMVECTOR& in_lockat, const DirectX::XMFLOAT3& in_up, const DirectX::XMFLOAT3& in_right)
    {
        // 向かせたいZ方向ベクトル
    ...
    DirectX::XMMATRIX LockAtMatrix(
        const DirectX::XMVECTOR& in_org, const DirectX::XMVECTOR& in_lockat, const DirectX::XMFLOAT3& in_up, const DirectX::XMFLOAT3& in_right)
    {
        // 1: Z軸ベクトルを特定方向に回転
        // 2: Z軸ベクトルをLockAtに回転
        // 3: 2の回転に1の逆回転を加える
