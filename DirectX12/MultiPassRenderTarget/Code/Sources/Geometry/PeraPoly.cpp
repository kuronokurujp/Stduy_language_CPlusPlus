...
namespace Geometry
{
        ...
        const std::string_view& in_r_shader_ps_filepath)
    {
        ...
        this->_mesh = std::make_shared<DirectX12::Mesh>(
            "pera_vs_buff" + Common::CreateGUIDString(), "pera_idx_buff" + Common::CreateGUIDString());
    ...
    {
        int v_count = _countof(PeraPoly::s_pv);
        std::vector<PeraPoly::Vertex> vertexs(v_count);
        {
