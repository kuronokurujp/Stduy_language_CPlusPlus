            ...
            std::vector<DirectX::XMMATRIX>& in_r_bone_matrices)
        {
            ...
            // マテリアルバッファを作成
            {
                ...
                // マテリアル数分のバッファ作成してデータ書き込み
                {
                    ...
                    UINT mem_size = static_cast<UINT>(alignment_size * this->_pmd_materials.size());
                    p_material_buff = DirectX12::CreateEmptyResourceByGPUTransition(in_context, this->_material_buff_key, mem_size);
                ...
                this->_material_desc_num = 0;
                {
                    ...
                    D3D12_CONSTANT_BUFFER_VIEW_DESC mat_desc = {};
                    {
                        ...
                        // 定数バッファの一塊サイズ
                        mat_desc.SizeInBytes = static_cast<UINT>(alignment_size);
                    }
                    ...
                    // マテリアルのデータ数 * (定数バッファ + テクスチャバッファ + スフィアテクスチャ + 加算スフィアテクスチャ)
                    {
                        ...
                        {
                            UINT32 desc_num = static_cast<UINT32>(this->_pmd_materials.size() * this->_material_desc_num);
            ...
            {
                UINT buff_size = static_cast<UINT>(sizeof(DirectX::XMMATRIX) * (in_r_bone_matrices.size() + 1));
