...
namespace DirectX12
{
    ...
    {
        auto buff_size = static_cast<UINT>(in_buff_size);
        auto size_in_bytes = static_cast<UINT>(in_size_in_bytes);
        ...
        // マテリアル数分のバッファ作成してデータ書き込み
        {
            ...
            p_material_buff = DirectX12::CreateEmptyResourceByGPUTransition(in_r_context, this->_constant_buff_key, buff_size);
        ...
        // マテリアル定数バッファビューの設定データ
        {
            ...
            // 定数バッファの一塊サイズ
            this->_mat_desc.SizeInBytes = size_in_bytes;
        }
    ...
    )
    {
        ...
        size_t now_desc_num = this->_tex_buff_view_desc_map.size();
        ...
        {
            UINT32 desc_num = static_cast<UINT32>(in_material_num * now_desc_num);
