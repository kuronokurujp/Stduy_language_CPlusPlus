...
namespace Common
{
    ...
    /// </summary>
    template <class T>
    static bool SafeRemoveObject(T* in_pObject, std::vector<T*>& in_rObjects)
    {
    ...
    {
        size_t idx = in_r_filename.find(in_spliter);
        ...
        const char* in_p_append_path)
    {
        ...
        INT path_index1 = static_cast<INT>(in_currnet_path.rfind('/'));
        INT path_index2 = static_cast<INT>(in_currnet_path.rfind('\\'));
