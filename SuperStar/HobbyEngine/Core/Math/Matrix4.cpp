...
namespace Math
{
    ...
    // 正射影行列作成
    Matrix4 Matrix4::CreateOrtho(const float in_width, const float in_height, const float in_near,
                                 const float in_far)
    {
        float temp[4][4] = {
            {2.0f / in_width, 0.0f, 0.0f, 0.0f},
    ...
    // 透視射影行列作成
    Matrix4 Matrix4::CreatePerspectiveFOV(const float in_fov, const float in_width,
