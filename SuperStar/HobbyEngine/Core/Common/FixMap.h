...
namespace Core
{
    namespace Common
    {
        ...
        /// <typeparam name="SIZE">SIZEで指定した値が最大要素数, 最大要素数を超えたらエラーとなる</typeparam>
        ...
        public:
            ...
            const Bool CheckNode(NODE* pNode, Uint32* pCheckedCount)
            {
                ...
                // 左ノードがあればコンペア
                if (pNode->_pLeft && this->_Compare(pNode->_pair._key, pNode->_pLeft->_pair._key) != -1)
                    return FALSE;

                // 右ノードがあればコンペア
                if (pNode->_pRight && this->_Compare(pNode->_pair._key, pNode->_pRight->_pair._key) != 1)
                    return FALSE;
