...
namespace Core
{
    namespace Time
    {
        FPS::FPS(Platform::TimeSystemInterface* in_pTimeInterface)
        {
            ...
            // 格納した時間をずらす
            for (Uint32 i = 0; i < FPS::_uTimeAvgMax - 1; ++i)
                this->_uaPreviousTime[i] = uCurrentTime;
        }
        ...
        const Float32 FPS::GetDeltaTimeMSec(Platform::TimeSystemInterface* in_pTimeInterface) const
        {
            ...
            const Float32 fOldMSec =
                static_cast<Float32>(this->_uaPreviousTime[FPS::_uTimeAvgMax - 1]);
            if (fOldMSec <= 0.0f) return 0.0f;

            const Float32 fNowMSec = static_cast<Float32>(in_pTimeInterface->NowMSec());
