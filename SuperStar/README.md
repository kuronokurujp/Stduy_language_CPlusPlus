...
-   手動整形
    ...
    -   ctrl + f でフォーカスしているファイルが整形<br>
        ※注意点<br>
        サードパーティーのコードをうっかり自動整形して改変をしないようにするため手動整形はしない
...
### 自動整形の非対象ディレクトリ
OSSのライブラリなどサードパーティーのコードは自動整形させない
...
#### 自動整形の非対象ディレクトリの作成手順
1. ThirdPartyディレクトリを作成
1. 自動整形したくないファイルをThirdPartyディレクトリに入れる
1. ThirdPartyディレクトリに.clang-format ファイルを作成
1. 作成したファイル内容を以下にする
    ```
    ---
    DisbaleFormat: true
...
#### protected or private は外部公開ができない事を読んで判断できるようにするためにメソッド名の先頭に_をつける
...
#### 宣言はoverrideで見て一目でわかるが、定義ではoverrideが付けれないので一目でわからず考える時間が生まれたから
```
    class Object
    {
    public:
...
#### クラスを派生しないなら必ずfinalを使う(finalを使っているなら継承はないので仮想デストラクタは必ず宣言する必要はない)
```
    class Object
    {
    public:
...
-   enum名のプレフィックスに E をつける
-   enumの要素の前にenum名を付ける
    - これはenumの名前のかぶりを避けるため
-   enumの要素名を参照する時はenum名をつけない
例)
