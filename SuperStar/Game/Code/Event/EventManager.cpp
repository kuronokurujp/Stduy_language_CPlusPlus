...
{
    EventManager::EventManager(std::unique_ptr<EventManagerStrategyInterface> in_rrStrategy)
        : Core::Common::Singleton<EventManager>(), _sActiveQueue(0)
    {
        this->_pStrategy = std::move(in_rrStrategy);
    }
    ...
    const Bool EventManager::VRelease()
    {
    ...
    const Bool EventManager::VValidateType(EventTypeStr const& in_rType) const
    {
        ...
        if (this->_pStrategy->VIsEventTypeStr(in_rType) == FALSE) return FALSE;
    ...
    const Bool SafeAddListener(EventListenerPtr const& in_rHandler, EventTypeStr const& in_rType)
    {
        return EventManager::I().VAddListenr(in_rHandler, in_rType);
    }

    const Bool SafeRemoveListener(EventListenerPtr const& in_rHandler, EventTypeStr const& in_rType)
    {
        return EventManager::I().VRemoveListener(in_rHandler, in_rType);
    }

    const Bool SafeTriggerEvent(EventDataInterfacePtr const& in_rEvent)
    {
        return EventManager::I().VTrigger(in_rEvent);
    }

    const Bool SafeQueueEvent(EventDataInterfacePtr const& in_rEvent)
    {
        return EventManager::I().VQueueEvent(in_rEvent);
    }

#if 0
    const Bool SafeAbortEvent(EventTypeStr const& in_rType, const Bool in_bAllOfType)
    {
        return EventManager::I().VAbortEvent(in_rType, in_bAllOfType);
    }
#endif

    const Bool SafeTickEventManager(const Uint32 in_uMaxMillis)
    {
        return EventManager::I().VTick(in_uMaxMillis);
    }

    // イベント型の有効性を確認
    const Bool SafeValidateEventType(EventTypeStr const& in_rType)
    {
        return EventManager::I().VValidateType(in_rType);
    }
