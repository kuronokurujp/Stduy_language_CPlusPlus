...
namespace MainSeq
{
	...
	public:
		...
		*/
		void	Update()
		{
			(this->*m_pUpdateFuncList[ m_State ])();
		}
	...
	private:
		...
		*/
		void	_DrawList()
		{
			m_pSpriteSelectedList->DebugDraw();
		}
		...
		*/
		void	_DrawView()
		{
			...
			GameLib::C_GameSystem& r =GameLib::C_GameSystem::inst();
			{
				...
				sprintf_s( pText, "AnimIdx[%d] / AnimIdxMax[%d]", m_pSprite->getIdxAnim(), m_pSprite->getMaxAnim() );
				r.drawDebugText( pText, 0, 64 );

				sprintf_s( pText, "Frame[%d] / FrameMax[%d]", m_pSprite->getAnimFrame(), m_pSprite->getAnimFrameMax() );
