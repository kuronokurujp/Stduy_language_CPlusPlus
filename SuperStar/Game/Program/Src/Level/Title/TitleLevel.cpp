...
namespace Level
{
    ...
    /*
        I_MainSeq* TitleLevel::update( C_Parent* ip_parent )
        {
            ...
            //	画面真ん中にタイトル名を表示
            r.drawDebugText( s_pTitleSeqTitleNameText, r.getScreenWidth() >> 1, r.getScreenHeight()
       >> 1 );

            std::string	keyGuideText	= s_pTitleSeqKeyGuideText;
            r.drawDebugText( s_pTitleSeqKeyGuideText, ( r.getScreenWidth() >> 1 ) - ( (
       keyGuideText.length() / 2 ) * 8 ), ( r.getScreenHeight() >> 1 ) + 32 );
