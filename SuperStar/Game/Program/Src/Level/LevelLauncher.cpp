...
namespace Level
{
    ...
    const Bool LevelLauncher::Begin()
    {
        ...
        {
            s_seqMap.Clear();
            s_seqMap.Add(E_STR_TEXT("C_TitleSeq"),
                         {E_STR_TEXT("TitleLevelName"), Core::Math::Vector3(100.0f, 50.0f, 0.0f),
                          Core::Math::Rect2(0, 0, 300, 50)});
            s_seqMap.Add(E_STR_TEXT("C_PlaySeq"),
                         {E_STR_TEXT("MainGameLevelName"),
                          Core::Math::Vector3(100.0f, 100.0f + 10.0f, 0.0f),
                          Core::Math::Rect2(0, 0, 300, 50)});
            s_seqMap.Add(E_STR_TEXT("C_SpriteDebugSeq"),
                         {E_STR_TEXT("SpriteTestLevelName"),
                          Core::Math::Vector3(100.0f, 150.0f + 20.0f, 0.0f),
                          Core::Math::Rect2(0, 0, 300, 50)});
            s_seqMap.Add(E_STR_TEXT("C_BossDebugSeq"),
        ...
        for (ST_SEQ_MAP::Iterator it = s_seqMap.Begin(); it != s_seqMap.End(); ++it)
        {
            Core::Common::Handle hBtnAndTextWidget =
                UI::UIModule::I().NewButtonAndText(it->_key.Str(), ++sortNo,
                                                   HOBBY_LOCALIZATION_MODULE.Text(E_STR_TEXT("JP"),
                                                                                  E_STR_TEXT(
                                                                                      "debug"),
                                                                                  it->_data.pName),
                                                   // it->_data.pName,
                                                   it->_data.rect, Render::WhiteColor.c,
                                                   it->_data.rect.Pos(), Render::BlackColor.c);
            ...
            // TODO: ボタンを押した後のイベント
            {
                UI::UIButtonComponent* pBtnComp = reinterpret_cast<UI::UIButtonComponent*>(
