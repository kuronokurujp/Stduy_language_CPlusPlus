...
void Renderer::_SetRenderColor(const Color32& in_rColor)
{
    ...
    glClearColor(in_rColor.r * invColorRate, in_rColor.g * invColorRate, in_rColor.b * invColorRate,
...
bool Renderer::_LoadShader()
{
    // スプライトシェーダー作成
    {
        ...
        if (!this->pSpriteShader->Load("Resources/Shaders/Sprite.vert",
    ...
    // シェーダーに正規正方系の座標変換の行列を渡す
    {
        ...
        auto viewProj = Math::Matrix4::CreateSimpleViewProj(static_cast<float>(this->width),
    ...
    // メッシュ用のシェーダー作成
    {
        ...
        // memo: shaderコードがs-jisでないとコード解析に失敗する！
        // ↓光の影響を受けないunlitシェーダー
        // if (!this->pMeshShader->Load("Resources/Shaders/BasicMesh.vert",
        // "Resources/Shaders/BasicMesh.frag"))
        // todo: 各特徴を持つシェーダークラスが必要
        // todo: 拡散反射光のシェーダーが必要
        if (!this->pMeshShader->Load("Resources/Shaders/Phong.vert",
        ...
        // 透視行列作成
        this->viewMtx = Math::Matrix4::CreateLookAt(Math::Vector3::Zero, Math::Vector3::Unit_X,
                                                    Math::Vector3::Unit_Z);
        this->projectionMtx =
            Math::Matrix4::CreatePerspectiveFOV(Math::DegreesToRadians(70.0f),
                                                static_cast<float>(this->width),
                                                static_cast<float>(this->height), 25.0f, 10000.0f);
