...
auto Mesh::Load(const std::string& in_rFileName, Renderer* in_pRenderer) -> bool
{
    ...
    // テクスチャ情報取得
    {
        ...
        if (!documentTextures.IsArray() || documentTextures.Size() <= 0)
        {
            ...
            SDL_Log(
                "Mesh File Textures has not documentTextures, there should be at least one => %s",
                in_rFileName.c_str());
            return false;
        }
    ...
    std::vector<float> vertices;
    {
        ...
        {
            // 少なくとも一つは必ず頂点がある！
            SDL_Log("Mesh File has not vertices, there should be at least one => %s",
                    in_rFileName.c_str());
            return false;
        }
        ...
        for (rapidjson::SizeType i = 0; i < verticesJson.Size(); ++i)
        {
            ...
            Math::Vector3 pos(static_cast<float>(vert[0].GetDouble()),
    ...
    std::vector<unsigned int> indices;
    {
        ...
        if (!indicesJson.IsArray() || indicesJson.Size() <= 0)
        {
            ...
            SDL_Log("Mesh File has not indices, there should be at least one => %s",
                    in_rFileName.c_str());
            return false;
        }
    ...
    this->pVertexArray =
        new VertexArray(vertices.data(),
                        static_cast<unsigned>(vertices.size() / VertexArray::ItemMax),
