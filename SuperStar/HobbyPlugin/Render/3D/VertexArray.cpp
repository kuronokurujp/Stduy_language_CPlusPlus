	...
	unsigned numIndices)
{
	...
	// データレイアウト作成
	{
		...
		glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, static_cast<GLsizei>(strideSize), reinterpret_cast<void *>(layoutOffsetPointerSize));
		layoutOffsetPointerSize += sizeof(float) * 3;
		...
		// xyzの頂点データ分アドレスをオフセットずらしている
		glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, static_cast<GLsizei>(strideSize), reinterpret_cast<void *>(layoutOffsetPointerSize));
	}
}
