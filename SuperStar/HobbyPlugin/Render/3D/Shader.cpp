...
// シェーダーをコンパイル
auto Shader::_CompileShader(const std::string& in_rFileName, GLenum in_shaderType,
...
auto Shader::_ValidateUniform(const char* in_pUniformName, const char* in_pAttrName) -> void
{
    switch (glGetError())
    {
        ...
        {
            SDL_Log("shader uniform invalid value: %s uniform name => %s", in_pUniformName,
                    in_pAttrName);
            break;
        }
        case GL_INVALID_OPERATION:
        {
            SDL_Log("shader uniform invalid operation: %s uniform name => %s", in_pUniformName,
                    in_pAttrName);
            break;
        }
        default:
