...
SpriteComponent::SpriteComponent(Actor* in_pOwner, const int in_drawOrder, GraphicData& in_rGraphicData) :
	Component(in_pOwner, 100)
{
...
auto SpriteComponent::Draw() -> void
{
	...
	auto world = Math::Matrix4::CreateScale(static_cast<float>(pTexture->GetWidth()), static_cast<float>(pTexture->GetHeight()), 1.0f);
	{
