...
InputComponent::InputComponent(Actor* in_pOwner) :
	MoveComponent(in_pOwner, 10)
{
	this->_Clear();
}
...
auto InputComponent::ProcessInput(const uint8_t* in_pKeyState) -> void
{
	...
	// 回転
	{
		...
		// 反時計周りで回転のキー入力
		if (this->counterClockWiseKeyCode != NoneKeyCode && in_pKeyState[this->counterClockWiseKeyCode])
		{
			angular += this->maxAngularSpeed;
		}
